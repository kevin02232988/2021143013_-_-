...
위 실험들을 통해서 호텔리뷰와 호텔 평점에 대한 **상관관계**를 확인해 보고, 정말로 두 지표가 **유의미**한지 검증해보는 시간이 될 수 있을 것으로 기대합니다.
...
- 평소에 필자는 국내/외 여행을 많이 다니면서, 호텔과 숙소들의 평점과 리뷰가 어떤 상관관계에 있는지, 정말로 그 정보는 믿을 수 있는지에 대한 의구심이 있었기에 이번 프로젝트를 통해 스스로 그 질문에 답을 해보는 시간이 될 수 있으거라 생각해, 이 주제와 데이터로 선택하였습니다.
...
- 중복 데이터를 삭제하고 위 데이터에서 영어 이 외에 언어를 제외하였습니다.
- 특수문자 도배 또한 제외하였습니다. 뿐만아니, 너무 과거의 데이터와 너무 최근의 데이터는 삭제하고 진행했습니다.
...
---
## 13. 🚀 느낀 점과 배운 점 
이번 프로젝트를 통해 Accuray를 높이기 위해서는 **데이터 전처리**와 **데이터의 일관성**이 모델 성능에 얼마나 큰 영향을 미치는지를 실감하며 모델 학습에 매우 중요한 요소임을 깨달을 수 있었습니다.
또한, 모델이 예측한 평점이 실제 평점과 유사함을 확인함으로써, **리뷰를 일일이 읽지 않고도 호텔의 전반적인 품질을 가늠할 수 있다는 가능성**을 발견할 수 있었습니다.
뿐만아니라 이 결과는 평점에 따라 호텔을 선택하는 것이 굳이 리뷰를 보지 않아도 좋은 선택을 할 수 있다는 방증이 됩니다.
