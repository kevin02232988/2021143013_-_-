...
이 프로젝트는 **호텔 리뷰 데이터셋**을 기반으로 리뷰 텍스트로부터 **긍정/부정 감성**을 예측하고, 이를 통해 전체 리뷰의 평균 평점과 모델이 예측한 평점 간의 관계를 분석한.
...
### 4. 모델 로드
Google의 사전 학습된 MobileBERT(`google/mobilebert-uncased`) 모델을 호텔 리뷰 감성 분석에 맞게 파인튜닝했다다.  
...
파인튜닝된 MobileBERT 모델을 사용하여 전체 리뷰 데이터에 대해 예측을 수행하고, 아래와 같은 결과를 얻음음.
...
전체 샘플에 파인튜닝을 진행하지 않은 경우 / 실제 평균 평점과 MobileBERT 모델이 예측한 평점 간 비교입니다.
![그래프](2.png)
...
전체 학습용 10% 샘플의 실제 평균 평점과 MobileBERT 모델이 예측한 평점 간 비교입니다.

![그래프](Figure_1.png)

Estimate Outcome 과 Actual Outcome의 차이를 그래프를 통해 확인한 결과, 데이터를 학습한 이후에 차이가 더 미미해 졌다는 것을 알 수 있습니다.
...
다양한 사전 학습 모델 실험: DistilBERT, RoBERTa 등 비교 실험 등등 을 해봄직 합니다.

Branch (지점) 별 분석: 지점별 예측 정확도 분석 및 개선할 수 있으 듯 합니다.

오분류 분석을 통해서 모델이 어떤 유형의 리뷰에서 자주 실수하는지 파악이 가능해 보입니다. (예: 비꼬는 어투, 중립적인 단어 등에서 실수 가능성)
...
이는 리뷰 기반 고객 만족도 모니터링, 자동화된 서비스 품질 평가 등에 활용 가능함을 시사합니다. 또한 데이터의 신뢰성을 바탕으로 다양한 2차적**활용**또한 시도해 봄 직함.
