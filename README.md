...
이 프로젝트는 **호텔 리뷰 데이터셋**을 기반으로 리뷰 텍스트로부터 **긍정/부정 감성**을 예측하고, 이를 통해 전체 리뷰의 평균 평점과 모델이 예측한 평점 간의 관계를 분석함함.
...
위 실험들을 통해서 호텔리뷰와 호텔 평점에 대한 **상관관계**를 확인해 보고, 정말로 두 지표가 **유의미**한지 검증해보는 시간이 될 수 있을 것으로 기대함.
...
- 특수문자 도배 또한 제외하였습니다. 뿐만아니, 너무 과거의 데이터와 너무 최근의 데이터는 삭제하고 진행함함.
...
이후, 학습된 모델을 이용해 전체 리뷰 데이터에 대해 긍정/부정 예측을 수행함함.
...
## 5. 리뷰 예측
리뷰 텍스트를 토크나이징하여 모델에 입력하고, 각 리뷰에 대해 긍정/부정 예측을 수행한다다.
```bash
...
## 7.상관계수 계산
실제 평점과 예측 평점 간의 상관관계를 계산하여 신뢰도를 분석한다다.
```bash
...
### 9_0)  분석 결과
파인튜닝된 MobileBERT 모델을 사용하여 일부분 리뷰 데이터에 대해 예측을 수행하고, 아래와 같은 결과를 얻었다.
...
지점별 실제 평점과 예상 평점의 관계를 시각화하여, 모델 예측이 실제 평점과 어느 정도 일치하는지 확인할 수 있다다.
...
Estimate Outcome 과 Actual Outcome의 차이를 그래프를 통해 확인한 결과, 데이터를 학습한 이후에 차이가 더 미미해 졌다는 것을 알 수 있다다.
