...
이 프로젝트는 **호텔 리뷰 데이터셋**을 기반으로 리뷰 텍스트로부터 **긍정/부정 감성**을 예측하고, 이를 통해 전체 리뷰의 평균 평점과 모델이 예측한 평점 간의 관계를 분석한다.

Google의 MobileBERT 모델을 **직접 파인튜닝**하여 감성 분류에 활용하고, 예측된 긍정 비율로부터 **예상 평점**을 도출한다.


##  프로젝트 목표

위 실험들을 통해서 호텔리뷰와 호텔 평점에 대한 **상관관계**를 확인해 보고, 정말로 두 지표가 **유의미**한지 검증해보는 시간이 될 수 있을 것으로 기대 가능함.
...
## **2_1. 이 데이터와 주제를 선택한 이유
- 평소에 필자는 국내/외 여행을 많이 다니면서, 호텔과 숙소들의 평점과 리뷰가 어떤 상관관계에 있는지, 정말로 그 정보는 믿을 수 있는지에 대한 의구심이 있었기에 이번 프로젝트를 통해 스스로 그 질문에 답을 해보는 시간이 될 수 있으거라 생각해, 이 주제와 데이터로 선택하였다.
...
- 중복 데이터를 삭제하고 위 데이터에서 영어 이 외에 언어를 제외함.
- 특수문자 도배 또한 제외하였습니다. 뿐만아니, 너무 과거의 데이터와 너무 최근의 데이터는 삭제하고 진행함.
...
###  2. 디바이스 설정
사전 세팅
Device 설정: GPU를 사용할 수 있으면 CUDA를 이용하고, 그렇지 않으면 CPU를 사용함.
...
### 3. 데이터 로드 및 라벨링
CSV 파일을 로드하고, 결측치를 제거한 후, Rating을 실수형으로 변환하고, 긍정/부정 라벨링을 진행함.
```bash
...
### 4. 모델 로드
Google의 사전 학습된 MobileBERT(`google/mobilebert-uncased`) 모델을 호텔 리뷰 감성 분석에 맞게 파인튜닝했다.  
이후, 학습된 모델을 이용해 전체 리뷰 데이터에 대해 긍정/부정 예측을 수행함.
...
---
### 4_1) 모델 파인튜닝 과정
Google의 사전 학습 모델 **google/mobilebert-uncased**를 기반으로, 감정 분류 목적에 맞게 파인튜닝을 수행했다.
학습 데이터 2,000개 샘플을 사용하여 3 에폭 동안 MobileBERT를 파인튜닝했다.
...
훈련된 모델과 토크나이저는 mobilebert_finetuned_drug/ 디렉토리에 저장 후 추론에 활용했다.

----

### 4_2) 모델 파인튜닝 과정/ 모델 학습 및 저장
 총 2,000개의 리뷰를 기반으로 MobileBERT(`google/mobilebert-uncased`)를 fine-tuning했다.
- 평점 기준으로 리뷰를 감성(긍정/부정)으로 변환했으며, 평점이 3인 중립 리뷰는 제외했다.
- 학습 중 훈련 및 검증 정확도를 기록하고, 각 epoch의 정확도를 시각화한 그래프(`accuracy_curve.png`)를 저장함.
- 학습된 모델과 토크나이저는 `mobilebert_custom_model_review` 디렉토리에 저장됩니다.
- 학습 로그는 `training_log.csv`로 저장되어, 각 epoch마다 손실 및 정확도를 추적 가능함.
---

## 5. 리뷰 예측
리뷰 텍스트를 토크나이징하여 모델에 입력하고, 각 리뷰에 대해 긍정/부정 예측을 수행한다.
```bash
...
## 6. 지점별 평점 계산
각 지점별로 실제 평점의 평균을 계산하고, 예측된 긍정 비율을 기반으로 예상 평점을 계산한다.
```bash
...
![Validation Accuracy Graph](hist_actual_ratings.png)

---


## 7.상관계수 계산
실제 평점과 예측 평점 간의 상관관계를 계산하여 신뢰도를 분석한다.
```bash
...
## 9. 📈 분석 결과
상관계수 분석
상관계수: 실제 평점과 예측 평점 간의 상관관계를 계산하여, 예측 모델의 신뢰도를 분석한다.
...
### 9_0)  분석 결과
파인튜닝된 MobileBERT 모델을 사용하여 일부분 리뷰 데이터에 대해 예측을 수행하고, 아래와 같은 결과를 얻을 수 있었다.
...
파인튜닝된 MobileBERT 모델을 사용하여 전체 리뷰 데이터에 대해 예측을 수행하고, 아래와 같은 결과를 얻음.
...
```
이 방식은 긍정 비율을 기반으로 1~5 사이의 예상 평점을 도출하는 선형 추정 방식이다.
...
지점별 실제 평점과 예상 평점의 관계를 시각화하여, 모델 예측이 실제 평점과 어느 정도 일치하는지 확인할 수 있다.




## 10_1)  그래프 시각화로 비교 (데이터 미 학습시/파인튜닝을 진행하지 않은 경우)

전체 샘플에 파인튜닝을 진행하지 않은 경우 / 실제 평균 평점과 MobileBERT 모델이 예측한 평점 간 비교.
![그래프](2.png)

----

## 10_2) 그래프 시각화로 비교 (일부 데이터를 학습시(10%))

전체 학습용 10% 샘플의 실제 평균 평점과 MobileBERT 모델이 예측한 평점 간 비교입.

![그래프](Figure_1.png)

Estimate Outcome 과 Actual Outcome의 차이를 그래프를 통해 확인한 결과, 데이터를 학습한 이후에 차이가 더 미미해 졌다는 것을 알 수 있다.

----

## 10_3) Confushion Matrix (혼동 행렬)

![그래프](confusion_matrix.png)




## 11.🚀 개선 방안
클래스 불균형 처리: 긍/부정 리뷰 비율이 불균형할 경우, oversampling / undersampling 또는 가중치 조정 필요하다고 보여진다.

정규화된 다중 클래스 평점 예측: 1~5 점수를 그대로 예측하는 방식으로 확장 가능하다.

다양한 사전 학습 모델 실험: DistilBERT, RoBERTa 등 비교 실험 등등 을 해봄직 하다.

Branch (지점) 별 분석: 지점별 예측 정확도 분석 및 개선할 수 있으 듯 하다.

오분류 분석을 통해서 모델이 어떤 유형의 리뷰에서 자주 실수하는지 파악이 가능해 보인다. (예: 비꼬는 어투, 중립적인 단어 등에서 실수 가능성)
...
본 프로젝트는 MobileBERT를 효과적으로 파인튜닝하여 호텔 리뷰의 긍정/부정 감성을 예측하고, 모델 예측 기반 평점이 실제 평점과 근접함을 확인함.
이는 리뷰 기반 고객 만족도 모니터링, 자동화된 서비스 품질 평가 등에 활용 가능함을 시사한다. 또한 데이터의 신뢰성을 바탕으로 다양한 2차적**활용**또한 시도해 봄 직함.

---
## 13.  느낀 점과 배운 점 
이번 프로젝트를 통해 Accuray를 높이기 위해서는 **데이터 전처리**와 **데이터의 일관성**이 모델 성능에 얼마나 큰 영향을 미치는지를 실감하며 모델 학습에 매우 중요한 요소임을 깨달을 수 있었음.
또한, 모델이 예측한 평점이 실제 평점과 유사함을 확인함으로써, **리뷰를 일일이 읽지 않고도 호텔의 전반적인 품질을 가늠할 수 있다는 가능성**을 발견 가능했음..
뿐만아니라 이 결과는 평점에 따라 호텔을 선택하는 것이 굳이 리뷰를 보지 않아도 좋은 선택을 할 수 있다는 방증이 된다.
